          ...
          <table class="highlight responsive-table">
            ...
            <tbody>
              <tr v-for="item in listaUsuarios.data" :key="item.id">
                ...
                <td>{{item.email}}</td>
                <td>
                  <router-link :to="'/admin/user-teacher/editar/'+item.id">
...
export default {
  ...
  methods: {
    atualizaPagina(){
        this.$http
        .get(this.$urlAPI+`usuarios/paginada/4`, {"headers":{"authorization":"Bearer "+this.$store.getters.getToken}})
        .then((response) => {
    ...
    cadastro(){
      ...
      this.$http.post(this.$urlAPI+`usuario/teacher/cadastro`, {
  ...
  computed:{
    listaUsuarios(){
      return this.$store.getters.getUsuarioTeacher;
    }
  }
