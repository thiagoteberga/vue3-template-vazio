      ...
      <card-tab-vue
        ...
        <span slot="tabela">
          <table class="highlight responsive-table">
            ...
            <tbody>
              <tr v-for="item in listaLivrosProfessor.data" :key="item.id">
                <td>{{item.id}}</td>
                <td>{{item.title}}</td>
                <td>{{item.description}}</td>
                <td>{{item.active == 1 ? 'Sim' : 'Não'}}</td>
                <td>
                  <router-link :to="'/admin/book-teacher/editar/'+item.id">
        ...
        <span slot="pagination">
          ...
          <span v-for="(item, index) in listaLivrosProfessor.links" :key="index">
        ...
        <span slot="inputs">
          <form v-on:submit.prevent="cadastro()" enctype="multipart/form-data">
            <div class="input-field col s12">
              ...
              <input id="title" type="text" class="validate" maxlength="100" v-model="bookTeacher.title">
...
export default {
  ...
  data () {
    CardTabVue
    return {
    bookTeacher: {title:'', description:'', active:'', link:''},
    carregando: false,
    percentualCarregado: ''
    }
  },
  ...
  methods: {
    atualizaPagina(){
        this.$http
        .get(this.$urlAPI+`teacher/livro/paginada`, {"headers":{"authorization":"Bearer "+this.$store.getters.getToken}})
        .then((response) => {
    ...
    salvaPdf(e){
      ...
      if(!pdfFile.length){
        ...
        this.bookTeacher.link = pdfFile[0];
      }
    },
    ...
    cadastro(){
      ...
      formData.append("link", this.bookTeacher.link);

      this.carregando = true;
      //this.percentualCarregado = '50';

      this.$http.post(this.$urlAPI+`teacher/livro/cadastro`,
                      formData,
                      {
                        "headers": headers,
    ...
    deletar(id){
      ...
      this.$http.delete(this.$urlAPI+`teacher/livro/delete/`+id, {"headers":{"authorization":"Bearer "+this.$store.getters.getToken}})
  ...
  computed:{
    listaLivrosProfessor(){
      return this.$store.getters.getBookTeacher;
    },
  }
