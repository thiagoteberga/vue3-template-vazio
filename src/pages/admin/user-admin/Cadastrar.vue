<template>
  <admin-template>
    ...
    <span slot="breadcrumb-template">
          ...
          <router-link to="/admin/user-admin/cadastrar" class="breadcrumb">Gerenciar Administrador</router-link>
    </span>
          ...
          <table class="highlight responsive-table">
            ...
            <tbody>
              <tr v-for="item in listaUsuarios.data" :key="item.id">
                ...
                <td>{{item.email}}</td>
                <td>
                  <router-link :to="'/admin/user-admin/editar/'+item.id">
...
export default {
  ...
  data () {
    ...
    return {
    isActive : false,
    usuario:{name:'', cpf:'', email:'', password:'', password_confirmation:''}
    }
  },
  mounted () {
    M.AutoInit()
  ...
  methods: {
    atualizaPagina(){
        this.$http
        .get(this.$urlAPI+`usuarios/paginada/3`, {"headers":{"authorization":"Bearer "+this.$store.getters.getToken}})
        .then((response) => {
          console.log("Retorno Recebido da API Lista!");

          if(response.status){
            this.$store.commit('setUsuarioAdmin',response.data.usuarios);
          }
        })
        .catch((e) => {
    ...
    cadastro(){
      ...
      this.$http.post(this.$urlAPI+`usuario/admin/cadastro`, {
  ...
  computed:{
    listaUsuarios(){
      return this.$store.getters.getUsuarioAdmin;
    }
  }
