      ...
      <card-tab-vue
        ...
        <span slot="tabela">
          <table class="highlight responsive-table">
            ...
            <tbody>
              <tr v-for="item in listaLicoes.data" :key="item.id">
                <td>{{item.id}}</td>
                <td>{{item.unidade.livro.title}}</td>
                <td>{{item.unidade.title}}</td>
                <td>{{item.title}}</td>
                <td>
                  <router-link :to="'/admin/licao/editar/'+item.id">
        ...
        <span slot="pagination">
          ...
          <span v-for="(item, index) in listaLicoes.links" :key="index">
        ...
        <span slot="inputs">
          ...
          <div class="input-field col s12">
            <select v-model="licao.book_id" @change="selecionaLivro">
          ...
          <div class="input-field col s12">
            ...
            <label for="description">Nome da Lição</label>
          </div>
...
export default {
  ...
  methods: {
    /*
    selecionaLivro() {
        const idLivroSelecionado = this.licao.book_id;
        const livroSelecionado = this.$store.getters.getUnidadeByBookId(idLivroSelecionado);
        this.$store.commit('setUnidade',livroSelecionado);
    },
    */

    atualizaPagina(){
        this.$http
        .get(this.$urlAPI+`licoes/paginada`, {"headers":{"authorization":"Bearer "+this.$store.getters.getToken}})
        .then((response) => {
          console.log("Retorno Recebido da API Lista Licoes!");
        ...
        //Unidades
        this.$http
        .get(this.$urlAPI+`unidades`, {"headers":{"authorization":"Bearer "+this.$store.getters.getToken}})
        .then((response) => {
    ...
    goToPage($pagina){
        ...
        .then((response) => {
          console.log("Retorno Recebido da API Lista Lições!");
          if(response.data.status){
            this.$store.commit('setLicao',response.data.licoes);
          }
        })
    ...
    cadastro(){
      this.$http.post(this.$urlAPI+`licao/cadastro`, {
      ...
      .then(response => {
          ...
          if(response.data.status){
              this.atualizaPagina();
              //this.$store.commit('setLicao',response.data.licoes);
              M.toast({html: 'Lição cadastrada com sucesso!', classes: 'teal darken-1 borda'});
    ...
    deletar(id){
      ...
      this.$http.delete(this.$urlAPI+`licao/delete/`+id, {"headers":{"authorization":"Bearer "+this.$store.getters.getToken}})
      ...
      .then(response => {
          ...
          if(response.data.status){
              this.atualizaPagina();
              //this.$store.commit('setLicao',response.data.licoes);
              M.toast({html: 'Lição excluída com sucesso!', classes: 'teal darken-1 borda'});
          }else if(response.data.status == false && response.data.validacao == true){
              console.log('Erro de Validacao');
              for (let erro of Object.values(response.data.erros)){
                M.toast({html: erro, classes: 'deep-orange accent-4 borda'});
